-- ============================================================
-- Mjey Billiard Club — полный setup для Supabase
-- Запусти этот файл целиком в SQL Editor
-- ============================================================
-- После запуска создаётся:
--   * 8 таблиц + RLS
--   * триггер автосоздания профиля
--   * демо-столы и демо-турниры
--   * админ-аккаунт:
--       email:    mjey@mjey.com
--       пароль:   Mjey25.05.06
--       username: Mjey
...
create policy "tables_admin_write" on public.billiard_tables for all using (public.is_admin()) with check (public.is_admin());

drop policy if exists "bookings_select_own_or_admin" on public.bookings;
create policy "bookings_select_own_or_admin" on public.bookings for select using (auth.uid() = user_id or public.is_admin());
...
create policy "bookings_update_own_or_admin" on public.bookings for update using (auth.uid() = user_id or public.is_admin());
...
create policy "sessions_admin_write" on public.live_sessions for all using (public.is_admin()) with check (public.is_admin());
...
create policy "regs_select_own_or_admin" on public.tournament_registrations for select using (auth.uid() = user_id or public.is_admin());
drop policy if exists "regs_insert_own" on public.tournament_registrations;
create policy "regs_insert_own" on public.tournament_registrations for insert with check (auth.uid() = user_id);
drop policy if exists "regs_update_own_or_admin" on public.tournament_registrations;
create policy "regs_update_own_or_admin" on public.tournament_registrations for update using (auth.uid() = user_id or public.is_admin());

drop policy if exists "fines_select_own_or_admin" on public.fines;
create policy "fines_select_own_or_admin" on public.fines for select using (auth.uid() = user_id or public.is_admin());
drop policy if exists "fines_admin_write" on public.fines;
create policy "fines_admin_write" on public.fines for all using (public.is_admin()) with check (public.is_admin());

drop policy if exists "payments_select_own_or_admin" on public.payments;
create policy "payments_select_own_or_admin" on public.payments for select using (auth.uid() = user_id or public.is_admin());
...
values
  ('Стол №1', 'pool',     200, 15, 20),
  ('Стол №2', 'pool',     200, 40, 20),
  ('Стол №3', 'pool',     200, 65, 20),
  ('Стол №4', 'snooker',  300, 15, 55),
  ('Стол №5', 'snooker',  300, 40, 55),
  ('Стол №6', 'russian',  250, 65, 55),
  ('Стол VIP', 'russian', 400, 40, 85)
...
-- ПРОВЕРКА
-- ============================================================
select 'Админ создан:' as info, id, email, username, role
from public.profiles
where email = 'mjey@mjey.com';
